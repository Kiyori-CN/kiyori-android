...
- Canonical black icon library source: `browser-icons`
...
- For future black icon buttons, prefer assets from `browser-icons` instead of Material icons or newly drawn replacements.
- Prefer `browser-icons/android-xml/` first for Android UI, because those assets can be copied directly into `app/src/main/res/drawable/`.
...
- If an exact icon is missing from `android-xml/`, use the matching asset from `browser-icons/png/` or `browser-icons/icons/` only when necessary.
