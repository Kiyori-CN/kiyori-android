...
构建与依赖版本统一维护在 `gradle/libs.versions.toml`。

## 技术文档

- [文档导航](docs/README.md)
- [2026-04-30 项目审计报告](docs/audit/project-audit-2026-04-30.md)
...
- [Android 权限矩阵](docs/security/permissions.md)
- [WebDAV 使用说明](docs/guides/webdav.md)
- [.nomedia 支持说明](docs/guides/nomedia.md)

## 当前架构状态

- 当前仍是单模块 `:app` 工程，先通过功能包边界维持可维护性，不急于照搬 `hikerView` 的多模块拆分。
- 主页底部五入口由 `HomeScreen` 管理；主页内部三页横滑，负一屏为工具/历史聚合页，中间页为浏览器优先主页，正一屏挂载 `operitreplica` AI 对话复刻页。
- Operit 相关代码当前是 UI 复刻和桥接组件，不是完整 Operit 运行时接入；Kiyori 自身状态集中在 `com.android.kiyori.operitreplica`。
- 播放器能力仍由 `libmpv` 和 `VideoPlayerActivity` 承担，是浏览器嗅探、WebDAV、本地媒体和远程 URL 的统一播放落点。
