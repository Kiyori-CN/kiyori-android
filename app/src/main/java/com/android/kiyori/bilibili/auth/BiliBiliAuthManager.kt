package com.android.kiyori.bilibili.auth

import android.content.Context
import android.content.SharedPreferences
import com.android.kiyori.bilibili.model.*
import com.android.kiyori.utils.SecureStorage
...
class BiliBiliAuthManager(private val context: Context) {
    ...
    suspend fun pollQRCodeStatus(qrcodeKey: String): LoginResult = withContext(Dispatchers.IO) {
        try {
            ...
            com.android.kiyori.utils.Logger.d("BiliAuth", "Poll response code: ${response.code}")
            com.android.kiyori.utils.Logger.d("BiliAuth", "Poll response body: $body")
            ...
            com.android.kiyori.utils.Logger.d("BiliAuth", "Poll API code: ${apiResponse.code}, data.code: ${apiResponse.data?.code}, message: ${apiResponse.data?.message}")
            
            when (apiResponse.data?.code) {
                0 -> {
                    com.android.kiyori.utils.Logger.d("BiliAuth", "Login success! URL: ${apiResponse.data.url}")
                    
                    // 登录成功，先访问返回的URL来获取Cookie
                    apiResponse.data.url?.let { loginUrl ->
                        try {
                            com.android.kiyori.utils.Logger.d("BiliAuth", "Accessing login URL to get cookies...")
                            val followRequest = Request.Builder()
                                .url(loginUrl)
                                .get()
                                .build()
                            val followResponse = client.newCall(followRequest).execute()
                            com.android.kiyori.utils.Logger.d("BiliAuth", "Follow response code: ${followResponse.code}")
                            
                            // 重要：从跳转响应中保存Cookie
                            saveCookiesFromResponse(followResponse)
                            com.android.kiyori.utils.Logger.d("BiliAuth", "Cookies saved from follow response")
                    ...
                    com.android.kiyori.utils.Logger.sensitive("BiliAuth", "Cookies after login", getCookies().toString())
                    ...
                    // 敏感信息：使用Logger.sensitive保护Cookie
                    com.android.kiyori.utils.Logger.sensitive("BiliAuth", "Final cookies", getCookies().toString())
                    com.android.kiyori.utils.Logger.d("BiliAuth", "User info after fetch: ${getUserInfo()?.uname}")
                    LoginResult.Success
                }
                86101 -> {
                    com.android.kiyori.utils.Logger.d("BiliAuth", "Waiting for scan")
                    LoginResult.WaitingScan          // 未扫码
                }
                86090 -> {
                    com.android.kiyori.utils.Logger.d("BiliAuth", "Scanned, waiting confirm")
