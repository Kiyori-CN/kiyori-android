...
class SubtitleManager {
    ...
    fun addExternalSubtitle(
        ...
        return try {
            ...
            if (path != null) {
                com.android.kiyori.utils.Logger.d(TAG, "===== Adding external subtitle =====")
                com.android.kiyori.utils.Logger.d(TAG, "Path: $path")
                ...
                com.android.kiyori.utils.Logger.d(TAG, "sub-add command executed with 'select' flag")
                
                // 立即查询 track-list 状态
                val trackCount = MPVLib.getPropertyInt("track-list/count") ?: 0
                val currentSid = MPVLib.getPropertyString("sid") ?: "no"
                com.android.kiyori.utils.Logger.d(TAG, "Immediately after sub-add: track-list/count = $trackCount, current sid = $currentSid")
                ...
                for (i in 0 until trackCount) {
                    ...
                    com.android.kiyori.utils.Logger.d(TAG, "  Track[$i]: type=$type, id=$id, lang=$lang, title=$title")
                }
                
                com.android.kiyori.utils.Logger.d(TAG, "Successfully added external subtitle: $path")
                
                com.android.kiyori.utils.Logger.d(TAG, "===== End adding external subtitle =====")
                true
            } else {
    ...
    fun addExternalSubtitleFromPath(filePath: String): Boolean {
        return try {
            ...
            com.android.kiyori.utils.Logger.d(TAG, "===== Adding external subtitle from path =====")
            com.android.kiyori.utils.Logger.d(TAG, "Path: $filePath")
            ...
            com.android.kiyori.utils.Logger.d(TAG, "Successfully added external subtitle: $filePath")
            com.android.kiyori.utils.Logger.d(TAG, "===== End adding external subtitle =====")
            true
        } catch (e: Exception) {
        ...
        return try {
            when (uri.scheme) {
                "file" -> {
                    ...
                    uri.path?.let { path ->
                        if (File(path).exists()) {
                            com.android.kiyori.utils.Logger.d(TAG, "Using file:// path directly: $path")
                            path
                        } else {
                            com.android.kiyori.utils.Logger.w(TAG, "File does not exist: $path")
                            null
                        }
                    }
                }
                "content" -> {
                    ...
                    com.android.kiyori.utils.Logger.d(TAG, "content:// URI detected, need to copy to cache")
                    copyContentUriToFile(context, uri)
                }
                else -> {
                    // 尝试直接使用 URI
                    com.android.kiyori.utils.Logger.d(TAG, "Unknown scheme: ${uri.scheme}, trying direct URI")
                    uri.toString()
                }
            }
        } catch (e: Exception) {
    ...
    private fun copyContentUriToFile(context: Context, uri: Uri): String? {
        return try {
            ...
            com.android.kiyori.utils.Logger.d(TAG, "Copying content URI to persistent storage: ${subtitleFile.absolutePath}")
            ...
            if (subtitleFile.exists()) {
                com.android.kiyori.utils.Logger.d(TAG, "Subtitle file created successfully: ${subtitleFile.absolutePath} (${subtitleFile.length()} bytes)")
