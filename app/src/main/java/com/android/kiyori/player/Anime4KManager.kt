...
class Anime4KManager(private val context: Context) {
    ...
    fun initialize(): Boolean {
        // 如果已经初始化过，直接返回成功
        if (isInitialized) {
            com.android.kiyori.utils.Logger.d(TAG, "Anime4K already initialized, skipping")
            return true
        }
        ...
        return try {
            ...
            if (allFilesExist && glslFiles.isNotEmpty()) {
                // 所有文件已存在，无需复制
                com.android.kiyori.utils.Logger.d(TAG, "All ${glslFiles.size} shader files already exist, skipping copy")
                isInitialized = true
                return true
            }

            // 复制缺失的着色器文件
            com.android.kiyori.utils.Logger.d(TAG, "Copying ${glslFiles.size} shader files from assets")
            for (fileName in glslFiles) {
                copyShaderFromAssets(fileName)
            }

            isInitialized = true
            com.android.kiyori.utils.Logger.d(TAG, "Anime4K shaders initialized successfully")
            true
        } catch (e: Exception) {
            com.android.kiyori.utils.Logger.e(TAG, "Failed to initialize Anime4K shaders", e)
            false
        }
    }
