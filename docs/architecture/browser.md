...
本文档用于冻结 Kiyori 浏览器功能的首期架构、交互、开发流程与安全边界，作为后续逐步开发的基线文档。
...
- 不直接移植外部参考浏览器项目的整套实现，只参考其中适合当前项目的能力点
